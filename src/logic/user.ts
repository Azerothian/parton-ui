...
export const getClientsForUserSelectionPath =
  "models.User.edges.node.clients.edges.node";
export const getClientsForUserQuery = gql`
  query getClientsForUser(
    $userId: ID
    $pageSize: Int
    $cursor: String
    $where: GQLTQueryClientWhere
...
export function getClientsForUserResult(result: any) {
  if (!result.data.models) {
    return undefined;
  }
  return result.data.models.User.edges[0].node.clients;
}

export const addClientsToUserMutation = gql`
  mutation addClientsToUser($userId: ID, $clientIds: [ID]) {
    models {
      User(
        update: {
          where: { id: { eq: $userId } }
          input: { clients: { add: { id: { in: $clientIds } } } }
        }
      ) {
        id
      }
...
`;
export function addClientsToUserResult(res: any) {
  return res.data.models.User[0].id;
}
